...
export function ContactDock({ locale }: { locale: Locale }) {
  ...
  const whatsapp = `https://wa.me/${siteConfig.whatsapp}?text=${encodeURIComponent(isDriverPage
    ? (locale === "id" ? "Halo AutoRev, saya ingin berkonsultasi mengenai syarat pendaftaran dan skema setoran Founding Driver." : "Hi AutoRev, I would like to discuss the registration requirements and daily payment scheme for Founding Driver.")
    : isEquipmentPage
      ...
      : (locale === "id" ? "Halo AutoRev, saya ingin berdiskusi mengenai ketersediaan unit sewa untuk kebutuhan pribadi atau perusahaan." : "Hi AutoRev, I would like to discuss rental vehicle availability for personal or company needs."))}`;
  return <><div className={`contact-dock ${open ? "is-open" : ""}`}>
    ...
    <button onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? (locale === "id" ? "Tutup kontak" : "Close contact") : (locale === "id" ? "Hubungi AutoRev" : "Contact AutoRev")}>{open ? <X size={21}/> : <MessageCircle size={21}/>}<span>{open ? "" : locale === "id" ? "Hubungi AutoRev" : "Contact AutoRev"}</span></button>
  </div><div className="mobile-action-bar"><Link href={localizePath(locale, isDriverPage ? "/founding-driver#platforms" : isEquipmentPage ? "/equipment#catalog" : "/#vehicle-catalog")}><MobileIcon size={21}/><span>{isDriverPage ? (locale === "id" ? "Lihat Platform" : "View Platforms") : (locale === "id" ? "Buka Katalog" : "Open Catalog")}</span></Link><Link href={localizePath(locale, isDriverPage ? "/contact?type=driver" : "/contact?type=business")}><MessageCircle size={21}/><span>{locale === "id" ? "Hubungi Kami" : "Contact Us"}</span></Link></div></>;
}
