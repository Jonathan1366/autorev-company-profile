...
export function Navbar({ locale }: { locale: Locale }) {
  ...
  const moreLinks = [
    ...
    },
    {
      ...
      title: locale === "id" ? "Sistem Fleet RevAuto" : "RevAuto Fleet System",
  ...
  const links = [
    ...
    { href: localizePath(locale, "/autorev-rental"), label: locale === "id" ? "Sewa Mobil" : "Car Rental" },
    { href: localizePath(locale, "/autorev-business"), label: locale === "id" ? "Solusi Bisnis" : "Business Solutions" },
  ...
  return <header className={`${styles.header} site-header ${scrolled || open || moreOpen ? "site-header--solid" : ""}`} onMouseLeave={() => setMoreOpen(false)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setMoreOpen(false); }}>
    <div className="nav-shell">
      ...
      <div className="nav-actions">
        ...
        <Link className="button button--nav" href={localizePath(locale, "/contact?type=business")}>{locale === "id" ? "Hubungi Kami" : "Contact Us"}</Link>
    ...
    <AnimatePresence initial={false}>
      {moreOpen && <motion.div ref={morePanel} id="more-navigation" className={styles.mega} initial={reduceMotion ? false : { opacity: 0, y: -8, scale: .99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: .99 }} transition={{ duration: .24, ease: [0.22, 1, 0.36, 1] }} onMouseEnter={() => setMoreOpen(true)}>
        <div className={styles.intro}><span>AUTOREV MOBILITAS INDONESIA</span><h2>{locale === "id" ? "Lebih dari perjalanan." : "Beyond the journey."}</h2><p>{locale === "id" ? "Kenali AutoRev, temukan dukungan operasional, lalu kelola armada melalui teknologi yang terus berkembang." : "Meet AutoRev, find operational support, then manage your fleet through evolving technology."}</p><Link href={localizePath(locale, "/about")} onClick={() => setMoreOpen(false)}>{locale === "id" ? "Tentang AutoRev" : "About the company"}<ArrowRight size={18}/></Link></div>
    ...
    <div id="mobile-navigation" className={`mobile-menu ${open ? "mobile-menu--open" : ""}`} aria-hidden={!open}>
      ...
      <div className="mobile-menu__footer"><a href={`https://wa.me/${siteConfig.whatsapp}`} target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight size={14}/></a><Link href={localizePath(locale, "/contact?type=business")} onClick={() => setOpen(false)}>{locale === "id" ? "Hubungi Kami" : "Contact Us"}<ArrowUpRight size={14}/></Link></div>
    </div>
  </header>;
}
